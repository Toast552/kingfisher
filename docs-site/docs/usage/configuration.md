...
| `git`        | clone dir, keep-clones, repo-clone-limit, include-contributors, GHE / self-hosted GitLab API URLs |
...
### Step 1 — generate the config from your existing CLI command

Don't write the YAML by hand. Run your existing `kingfisher scan ...`
flags under `kingfisher config init` and capture the output:
...
`--config FILE` is required — Kingfisher does not auto-discover
`kingfisher.yaml`. You can override any config value on the CLI for a
single run; CLI always wins:
...
`endpoints` that don't follow `provider=url`. There is no auto-discovery —
pass `--config FILE` explicitly so a missing or malformed file fails fast
